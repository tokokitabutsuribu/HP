...
export default function () {
  useEffect(() => {
    ...
    async function getmytoken() {
      ...
      getToken(messaging, {
        vapidKey:
          "BHYfDERRzVeOZOz32LOi6uZTYpzItJ5MVK8EswEeYkjLLOeX8thI1o7yPBuizxXqq_j_r1pauCAo3_YTGWxc7tQ",
      })
        .then((currentToken) => {
          if (currentToken) {
    ...
    function requestPermission() {
      ...
      if (
        ...
        if (!window.matchMedia("(display-mode: standalone)").matches) {
          (document.getElementById("pop-up") as HTMLInputElement).checked =
            true;
        }
      }
        ...
        .then((permission) => {
          if (permission === "granted") {
            ...
            if (
              ...
              if (!window.matchMedia("(display-mode: standalone)").matches) {
                (
                  document.getElementById("pop-up") as HTMLInputElement
                ).checked = true;
              } else {
                document.getElementById("requestpermission").innerText =
                  "通知がブロックされています";
              }
            } else {
              document.getElementById("requestpermission").style.display =
