...
async function push_token(req) {
  ...
  await supabase
    ...
    .upsert({
      ...
      true_topics: request.true_topics,
      false_topics: request.false_topics,
    })
    .select()
    .then(({ statusText }) => {
  ...
  for (const topic of request.true_topics) {
    await messaging
      .subscribeToTopic(request.token, topic)
      .then((res) => {
        // See the MessagingTopicManagementResponse reference documentation
        // for the contents of response.
        errormessage.push(
          `Successfully subscribed to topic:${JSON.stringify(res)}`
        );
      })
      .catch((error) => {
  ...
  for (const topic of request.false_topics) {
    await messaging
      .unsubscribeFromTopic(request.token, topic)
      .then((res) => {
        // See the MessagingTopicManagementResponse reference documentation
        // for the contents of response.
        errormessage.push(
          `Successfully unsubscribed from topic:${JSON.stringify(res)}`
        );
      })
      .catch((error) => {
