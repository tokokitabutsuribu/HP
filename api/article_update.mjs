...
export default async (request, response) => {
    ...
    let condition = "'new-article' in topics ||'all' in topics";
    for (let i = 0; i < Math.min([3, request.body.contents.new.publishValue.category.length]); i++) {
        condition += "||'"+request.body.contents.new.publishValue.category[i] + "' in topics";
    }       
