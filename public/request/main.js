  ...
  document.getElementById("submit").addEventListener("click", () => {
    try {
      ...
      const musiclist = Array.from(
        document.getElementsByClassName("music")
      ).map((musicelem) => {
        const ret = {};
        Array.from(musicelem.children).map((childelem) => {
          ...
          ret[childelem.children[1].getAttribute("name")] =
            childelem.children[1].value;
        });
        return ret;
      });
      if (iserror) {
      ...
      fetch(apiurl, {
        ...
        body: JSON.stringify({ musiclist: musiclist }),
      })
        .then((data) => {
          rawdata = data;
          return data.json();
        ...
        .then((data) => {
          if (rawdata.ok) {
            ...
            console.log(
              `${rawdata.status}:${rawdata.statusText}\n${data.message}`
            );
          }
        })
        .catch((e) => {
